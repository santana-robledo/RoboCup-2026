...
class BallTracker:
    ...
    def read(self):
        ...
        if goal_x is not None:
            color_goal = (255, 0, 0) if COLOR_PORTERIA == "AZUL" else (0, 255, 255)
            cv2.rectangle(debug, (goal_x - 30, goal_y - 30),
                          (goal_x + 30, goal_y + 30), color_goal, 2)
            cv2.putText(debug, f"goal_{COLOR_PORTERIA} error_x={error_x_goal:+d} area={int(goal_area)}", (10, 165),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.65, color_goal, 2)
