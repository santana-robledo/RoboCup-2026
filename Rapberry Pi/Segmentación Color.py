...
error_y_int = 0.0
error_y_der=0.0
alpha = 0.7 #Filtro para suavizar derivada
...
while True:
    ...
    if estado == BUSCAR: #Verifica si la variable estado es igual a BUSCAR
        ...
        if ball_detected: #Si se detecta pelota cambiamos a perseguir
         ...
         error_x_int += error_x * dt #Acumula la suma de los errores en el tiempo
         error_x_int = np.clip(error_x_int, -1, 1) #Limita la suma acumulada de errores para que no crezca demasiado

         error_x_der = (error_x - error_x_prev) / dt #Calcula qué tan rápido está cambiando el error
         error_x_prev = error_x #Guarda el error actual para calcular el cambio en la siguiente iteración
         if a_p >= zona_ataque: #Verifica si la pelota ya está suficientemente cerca
            Ux = Ux_ataque #Velocidad constante
         else:
            Ux = Kp_x * error_x + Ki_x * error_x_int + Kd_x * error_x_der #si no calculo de correción de error
         ...
         if a_p >= zona_ataque: #Verifica si la pelota ya está suficientemente cerca del robot.
             estado = CONTROLAR #Cambia de estado
