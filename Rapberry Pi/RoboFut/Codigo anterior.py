    ...
    if estado == BUSCAR : #Verifica si la variable estado es igual a BUSCAR
        ...
        # Cambia de estado si se mantiene por buffer_transicion frames
         ...
         ##if not ball_detected:#Si la pelota dejo de detectarse vuelve a buscar
             ...
             # tiempo que lleva controlando
             tiempo_actual = time.time()
             tiempo_en_control = tiempo_actual - tiempo_controlar

             # mantener control x segundos, verificar en pruebas
             if tiempo_en_control >= 3.55:

                 if estado_pelota == 0:  # sensor dice que ya tiene pelota
                     estado = APUNTAR

                 if estado_pelota == 1:  # perdió pelota
                     estado = BUSCAR
# 
