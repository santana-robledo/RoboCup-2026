...
def detectar_porteria(mask, min_area=5000):  # Recibe mascara y solo considerara objetos mayores a min_area
    contornos, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL,
                                    cv2.CHAIN_APPROX_SIMPLE)  # Busca contornos en la imagen, solo contornos externos, CHAIN_APPROX_SIMPLEsimplifica puntos del contorno
...
while True:
    ...
    hsv = cv2.cvtColor(frame,
                       cv2.COLOR_BGR2HSV)  # Cambia BGR a HSV, mejor para segmentar colores (tono, saturación, brillo)
    ...
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL,
                                   cv2.CHAIN_APPROX_SIMPLE)  # Busca contornos blancos en la mascara, lista de objetos detectados
    ...
    if arduino:  # Verifica que el objeto arduino exista
        line = arduino.readline().decode(
            errors='ignore').strip()  # Lee una línea del puerto serial, la decodifica a texto y elimina espacios o saltos de línea.
    ...
    if mejor_contorno is not None:  # Si encuentra pelota
        (x, y), radius = cv2.minEnclosingCircle(
            mejor_contorno)  # Calcula el círculo mínimo que encierra el contorno y guarda el centro en (x, y)
        ...
        if radius > 10:  # Para evitar ruidos muy pqueños
            ...
            cv2.circle(frame, center, int(radius), (0, 255, 0),
                       2)  # Dibuja un círculo verde sobre frame con centro en center y radio radius
    ...
    if estado == BUSCAR:  # Verifica si la variable estado es igual a BUSCAR
        ...
        error_y_der = alpha * error_y_der + (1 - alpha) * ((
                                                                       error_y - error_y_prev) / dt)  # Calcula qué tan rápido cambia el error lateral, aplicando un filtro para suavizar el cambio.
